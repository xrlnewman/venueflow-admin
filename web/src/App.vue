...
const shipments = ref([
  { id: 'FF-260716-018', route: '浦东 → 静安', cargo: '生鲜 12 箱', driver: '周师傅', vehicle: '沪A·72K31', eta: '14:35', status: '现场服务中', tone: 'blue' },
  { id: 'FF-260716-017', route: '虹桥 → 徐汇', cargo: '餐饮食材 28 箱', driver: '陈师傅', vehicle: '沪B·18Q90', eta: '15:10', status: '待接单', tone: 'orange' },
  { id: 'FF-260716-016', route: '杨浦 → 宝山', cargo: '电商包裹 86 件', driver: '林师傅', vehicle: '沪C·39P06', eta: '已签收', status: '已完成', tone: 'green' },
  { id: 'FF-260716-015', route: '闵行 → 长宁', cargo: '办公物资 16 箱', driver: '王师傅', vehicle: '沪D·55L18', eta: '16:20', status: '待调度', tone: 'purple' },
  { id: 'FF-260716-014', route: '嘉定 → 普陀', cargo: '家居配件 42 件', driver: '赵师傅', vehicle: '沪E·03R88', eta: '17:05', status: '现场服务中', tone: 'blue' }
])
const exceptions = ref([
  { id: 'EX-041', type: '超时预警', text: 'FF-260716-018 预计晚到 18 分钟', level: '高', tone: 'red' },
  { id: 'EX-040', type: '地址确认', text: 'FF-260716-017 收货人电话无人接听', level: '中', tone: 'orange' },
  { id: 'EX-039', type: '场地资源告警', text: '沪C·39P06 需要补充冷链温度记录', level: '低', tone: 'blue' }
])
...
function toneFor(status) { return ({ '现场服务中': 'blue', '待接单': 'orange', '已完成': 'green', '待调度': 'purple', '已送达': 'blue', '已取消': 'red' })[status] || 'purple' }
...
async function advance(item) {
  const next = item.status === '现场服务中' ? '已送达' : item.status === '已送达' ? '已完成' : ''
...
async function createShipment() {
  const route = window.prompt('输入现场服务路线', '浦东 → 静安')
  if (!route) return
  const cargo = window.prompt('输入活动物料摘要', '日配活动物料 8 箱')
  if (!cargo) return
  try { await fleetApi.createShipment({ route, cargo, eta: '待安排' }); flash('新活动订单已创建，等待调度'); await refresh(); active.value = 'shipments' } catch (error) { flash(error.message) }
}
...
<template>
  <div class="shell">
    ...
    <main class="main">
      ...
      <section v-else-if="active === 'shipments'" class="panel full"><div class="panel-head"><div><h2>活动订单调度</h2><p>{{ shipments.length }} 条活动订单 · 创建、分配、现场服务、签收全流程</p></div><button class="primary small" @click="createShipment">＋ 新建活动订单</button></div><div class="table"><div class="table-head"><span>活动订单 / 路线</span><span>活动物料</span><span>场馆工作人员 / 场地资源</span><span>预计送达</span><span>状态</span><span>动作</span></div><div v-for="item in shipments" :key="item.id" class="table-row"><span><strong>{{ item.id }}</strong><small>{{ item.route }}</small></span><span>{{ item.cargo }}</span><span>{{ item.driver || '待分配' }}<small>{{ item.vehicle || '—' }}</small></span><span>{{ item.eta }}</span><span><b :class="['pill', item.tone]">{{ item.status }}</b></span><span><button v-if="item.status === '待接单' || item.status === '待调度'" class="text-action" @click="assign(item)">分配</button><button v-else-if="item.status === '现场服务中' || item.status === '已送达'" class="text-action" @click="advance(item)">{{ item.status === '现场服务中' ? '确认送达' : '完成签收' }}</button><button v-else class="text-action muted-action" @click="flash(`${item.id} 已完成闭环`)">已闭环</button></span></div></div></section>
      <section v-else-if="active === 'drivers'" class="panel full"><div class="panel-head"><div><h2>场地资源场馆工作人员</h2><p>58 辆接入场地资源 · 42 辆正在现场服务</p></div><button class="link" @click="flash('场地资源导出任务已创建')">导出名单 ↓</button></div><div class="driver-grid"><article v-for="(driver, index) in ['周师傅','陈师傅','林师傅','王师傅','赵师傅','孙师傅']" :key="driver" class="driver-card"><div class="driver-avatar">{{ driver[0] }}</div><div><strong>{{ driver }}</strong><small>{{ ['沪A·72K31','沪B·18Q90','沪C·39P06','沪D·55L18','沪E·03R88','沪F·61P72'][index] }}</small></div><span :class="['online', index === 3 ? 'busy' : '']">{{ index === 3 ? '休息中' : '现场服务中' }}</span><b>{{ [98, 96, 94, 88, 97, 91][index] }}<small> 分</small></b></article></div></section>
